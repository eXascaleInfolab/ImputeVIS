        ...
        <div>
          ...
          <table class="table" style="margin-bottom: 50px;">
            ...
            <tr>
              ...
              <th style="width: 10%;">SHAP Value</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="(value, key) in shapValues" :key="key">
              <td>{{ key }}</td>
...
// Function to explain model predictions using SHAP
    const explainModelWithSHAP = async () => {
      if (!features) {
        console.error('Features not available');
        return;
      }

      const model = await trainModel();

      // Convert features to a format suitable for SHAP
      const shapInput = shap.format.convertToDataset(features);

      // Explain model predictions using SHAP
      const shapValues = shap.explainer(model.predict).explain(shapInput);

      // Display SHAP values
      shapValuesRef.value = shapValues;
    };
    ...
    watch(features, explainModelWithSHAP, { immediate: true });

    const shapValuesRef = ref([]);
