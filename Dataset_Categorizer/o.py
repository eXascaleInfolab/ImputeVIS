...
def shap_tester(dataset_path: str, obfuscated_dataset_path: str, dataset: str, algorithm:str):
    """
    ...
    sample_indices = np.random.choice(X_train.shape[0], size=16, replace=False)  # Sample size of 3 is used as an example. Adjust as desired???
    ...
    print("\t\t### SET LABEL TRAIN (RMSE) BY SERIES ==================================\n\n")
    ...
    print("\t\t### COMPUTE THE REGRESSION MODEL ==================================\n\n")
    ...
    alpha = "./figs/"+algorithm+"_shap_summary_plot.png"
    plt.savefig(alpha)

    #shap.plots.beeswarm(shap_values, plot_size=(25,10), order=shap_values.abs.max(0))
    #beta = "./figs/"+algorithm+"_shap_beeswarm_plot.png"
    #plt.savefig(beta)

    #shap.plots.bar(shap_values, max_display=12)
    #gamma = "./figs/"+algorithm+"_shap_bar_plot.png"
    #plt.savefig(gamma)

    #shap.plots.waterfall(shap_values[0])
    #delta = "./figs/"+algorithm+"_shap_waterfall_plot.png"
    #plt.savefig(delta)
    print("\t\t=======================================\n\n")
...
for dataset, data_file in zip(datasets, dataset_files):
    ...
    for algorithm in ALGORITHMS:
        ...
        shap_values = shap_tester(raw_file_path, obf_file_path, dataset, algorithm)

        print(dataset + ": ",  len(shap_values), " - ", str(shap_values), "\n\n")

        shap_values = np.sort(shap_values)[::-1]

        for x, sv in enumerate(shap_values):
            p = int(sv*100)
            if p < 10:
                print("RESULT >> ", p ,"%  of impact for feature : ", FEATURES[x])
            else:
                print("RESULT >> ", p ,"% of impact for feature : ", FEATURES[x])
        print("\n")
